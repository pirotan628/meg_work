...
path_PhysAct = 'user-site-export'
path_SpO2 = 'user-site-export'
path_Sleep = 'user-site-export'
...
def main_routine():
    for dirname in dirlist:        #[Epw****] を順に探索
        pngbase = dirname + "_fitbit_use.png"
        pngfile = os.path.join(dirname, pngbase)   #PNGファイルのフルパスを生成
        
        path_fitbit = os.path.join(dirname,'fitbit_USE') #fitbitデータのある場所を検索

        if os.path.exists(path_fitbit) == False:  # "fitbit" フォルダが見つからない場合
            print(dirname, " doesn't have fitbit_USE directory.")
